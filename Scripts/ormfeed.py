...
for category in categories:
    url = f'''https://world.openfoodfacts.org/cgi/search.pl?search_terms=\
    {category.name}&search_simple=1&action=process&json=1&page={data['page']}&\
...
duplicates_store = (ses.query(Store).join(subq,
                    and_(Store.name == subq.c.name, Store.Id != subq.c.min_id))
                    )
